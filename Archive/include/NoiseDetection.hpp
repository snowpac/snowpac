...
class NoiseDetection : protected VectorOperations {
  ...
  public:
    NoiseDetection ( std::vector<TSurrogateModel> &surrogate_models_input,
                     double &delta_input,
                     int observation_span_input, 
                     int nb_allowed_noisy_iterations_input )
                   { delta = &delta_input;
                     surrogate_models = &surrogate_models_input;
                     threshold = 1e0; 
                     nb_surrogate_models = 0;
                   }
    void initialize_noise_detection ( int, int );
    void detect_noise ( int& );
...
//--------------------------------------------------------------------------------
template<class TSurrogateModel>
void NoiseDetection<TSurrogateModel>::initialize_noise_detection( 
...
//--------------------------------------------------------------------------------
template<class TSurrogateModel>
void NoiseDetection<TSurrogateModel>::reset_noise_detection( ) 
{
  tr_radii.clear();
...
//--------------------------------------------------------------------------------
template<class TSurrogateModel>
void NoiseDetection<TSurrogateModel>::detect_noise( int &EXIT_FLAG ) 
{
  ...
  for ( int i = 0; i < nb_surrogate_models; ++i ) { 
    ...
    if ( nb_observations == observation_span ) {
      ...
      if ( noisy_iterations_counter[i] > nb_allowed_noisy_iterations ) EXIT_FLAG = -2;
    }
  }
  return;
}
