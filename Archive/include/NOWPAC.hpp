...
class NOWPAC : protected NoiseDetection<TSurrogateModel> {
  ...
  public:
    ...
    double &get_c( int, double&, std::vector<double>& );
    std::vector<double> &get_g( int, double&, std::vector<double>& );
    ...
    std::vector< std::vector<double> > &H )
{
  ...
  c += dot_product( surrogate_model[model_number], evaluations.center_node) / evaluations.scaling;
  mat_vec_product( surrogate_model[model_number].hessian(), evaluations.center_node), g ); 
  c += 0.5*dot_product( evaluations.center_node, g ) / ( evaluations.scaling * evaluations.scaling );

  scale ( 1.0/(evaluations.scaling * evaluations.scaling), g, g);
  add( 1.0/evaluations.scaling, surrogate_model[model_number].gradient(), g);  

  H = surrogate_models[model_number].hessian();
  for ( int i = 0; i < dim; ++i ) {
    for ( int j = 0; j < dim; ++j ) {
      H[i][j] /= (evaluations.scaling * evaluations.scaling);
    }
  }
  ...
  ( int model_number, std::vector<double> &x )
{
  ...
  double c;
  c = surrogate_models[model_number].evaluate( evaluations.transform(x) );
  ...
  ( int model_number, std::vector<double> &x )
{
  ...
  mat_vec_product( surrogate_model[model_number].hessian(), evaluations.center_node), g ); 
  mat_vec_product( surrogate_model[model_number].hessian(), x, gtmp ); 

  c += 0.5*dot_product( evaluations.center_node, g ) / ( evaluations.scaling * evaluations.scaling );

  scale ( 1.0/(evaluations.scaling * evaluations.scaling), g, g);
  add ( 1.0/(evaluations.scaling * evaluations.scaling), gtmp, g);
  add( 1.0/evaluations.scaling, surrogate_model[model_number].gradient(), g);  
...
//        inner_boundary_path_constants.at(i) = max_inner_boundary_path_constants.at(i);
      ...
      tmp_dbl = pow( this->diff_norm( x_trial, evaluations.nodes[ evaluations.best_index ] ) / delta, 1e0 );
