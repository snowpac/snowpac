                                      ...
                                      std::vector<double> const &new_node ) 
{
  ...
  for (unsigned int i = 0; i < nb_nodes; ++i) {
    if ( evaluations.active_index[ i ] != reference_node ) {
      ...
      norm = diff_norm( evaluations.nodes[ evaluations.active_index[i]],
...
//    if ( print_output )
  ...
  //  intdelete = evaluations.active_index[ change_index ];
...
/*    
  ...
  if ( outputfile.is_open( ) ) {
    for (int i = 0; i < evaluations.active_index.size(); ++i) {
        outputfile << evaluations.nodes[evaluations.active_index[i]][0] << "; " << 
